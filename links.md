# Helpful Links

## Miscellaneous
 - [Web Applications 101](https://www.robinwieruch.de/web-applications/)
 - [LaunchCode JavaScript Course](https://education.launchcode.org/intro-to-professional-web-dev/index-full.html)
 - [App Services JavaScript Community](https://www.wwt.com/community/javascript-development/about)

## React

 - [React Docs (Beta)](https://beta.reactjs.org/)
 - [Typical Starting Place for Creating a React App](https://create-react-app.dev/)
 - [Udemy Lession on React + Redux](https://www.udemy.com/course/react-the-complete-guide-incl-redux/)
 - [React Hooks API Docs](https://reactjs.org/docs/hooks-reference.html)
 - [React Testing Library](https://testing-library.com/docs/react-testing-library/cheatsheet)
 - [React Developer Tools Extension](https://chrome.google.com/webstore/detail/react-developer-tools/fmkadmapgofadopljbjfkapdkoienihi)
 - [Simple React String Transformer App](https://github.wwt.com/hoekzemj/string-transformer)
 - [Fall 2021 Example React App](https://gitlab.wwt.com/fall_2021_apprentices/staffnado/-/tree/main)
 - [Redux Tutorial](https://redux.js.org/tutorials/essentials/part-1-overview-concepts)
 - [React Component Composition](https://frontendmastery.com/posts/advanced-react-component-composition-guide/)
 - [Server Side React with NextJS](https://nextjs.org/)
   - [Main Tutorial](https://nextjs.org/learn/basics/create-nextjs-app)
   - [YouTube Tutorial](https://www.youtube.com/watch?v=MFuwkrseXVE)
   - [Comparison to Remix](https://remix.run/blog/remix-vs-next)

## Node

 - [Example CRUD App (Snippets API)](https://github.com/wwt/lets-code-snippets-js)

## JavaScript

 - [JavaScript Info Site, lots of info](https://javascript.info)
   - [switch statements](https://javascript.info/switch)
   - [keys, values, and entries](https://javascript.info/keys-values-entries)
   - [Array Methods](https://javascript.info/array-methods)
   - [Asynchronous Code](https://javascript.info/async) + [What Color is Your Function?](https://journal.stuffwithstuff.com/2015/02/01/what-color-is-your-function/)
   - [Arrow Functions](https://javascript.info/arrow-functions)
 - [Using Fetch](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch)
 - [Cross-Origin Resource Sharing](https://www.wwt.com/article/what-is-cors-cross-origin-resource-sharing)
 - [TypeScript Overview](https://2ality.com/2018/04/type-notation-typescript.html)
 - [Some complex TypeScript Usage](https://www.youtube.com/watch?v=hBk4nV7q6-w)

## CSS

 - [Utility for visualizing nested Flex boxes (little trippy and unintuitive)](https://github.com/Hoekz/flexbox)
 - [Media Queries](https://developer.mozilla.org/en-US/docs/Web/CSS/@Media)
 - [Flexbox Froggy (game for learning CSS Flexbox)](https://flexboxfroggy.com/)
 - [Grid Garden (game for learning CSS Grid)](https://cssgridgarden.com/)

## Databases

 - [Find all posts with a set of tags (SQL)](https://stackoverflow.com/questions/3876240/need-help-with-sql-query-to-find-things-tagged-with-all-specified-tags)
 - [MongoDB Compass (DB Explorer)](https://www.mongodb.com/products/compass)

## Authentication

 - [Walkthrough of adding Authentication to a Remix App](https://portal.gitnation.org/contents/server-side-auth-with-remix-prisma-and-the-web-platform)
 - [JSON Web Tokens](https://jwt.io/)
 - [Fall 2021 Example Authentication App](https://gitlab.wwt.com/fall_2021_apprentices/authentication)
